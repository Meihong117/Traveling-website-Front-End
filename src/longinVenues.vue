<template>
  <div style="background-color: lavenderblush">
    <h2 style="color: red;font-family: cursive">after login</h2>
    <div class="top">
      <h2 style="font-family: cursive"><span>Search Venues:</span></h2>
<!--Button-->
      ...
      <Button type='primary' onclick="window.location.href='/LoginHome'" style="margin-left: 9px;" ><Icon type="ios-home"  style="font-size: 18px;"/></Button>
      <br>
    </div>
<!--Table-->
    <h2 style="font-family: cursive">Show Venues:</h2>
...
//不删
      show(index){  //index=0,1,2,3.....          // to oneVenue
        let id = this.data1[index].venueId;  //1, 2,
        this.$router.push({name:'AddReview', params:{venue_id: id}});
      },

//show1
      show1(index){  //index=0,1,2,3.....          // to oneVenue
        let id = this.data3[index].venueId;  //1, 2,
        let photo=this.data3[index].primaryPhoto;
        this.$router.push({name:'AfteroneVenue', params:{venue_id: id, photoName: photo}});
      },
